...
module Network.HTTP.UserAgent
    ( agents, UserAgent(..), randomAgent
    ) where
...
newtype UserAgent = UserAgent { unUserAgent :: Text.Text } deriving Show

-- | Select a random agent, this can for example be run in IO.
randomAgent ::  MonadRandom m => m (UserAgent)
randomAgent = sample $ randomElement $ Non.toList agents

-- | All the available user agents
agents :: Non.NonEmpty UserAgent
agents = fmap UserAgent $ "Mozilla/5.0 (Windows; U; Windows NT 6.0; en-US) AppleWebKit/527  (KHTML, like Gecko, Safari/419.3) Arora/0.6 (Change: )" Non.:| [
